...
# 람다 함수 정의 방법
lambda_add = lambda a, b: a + b # 함수명 = lambda 매개변수명: 리턴값 
...
sorted_students = sorted(students, key=lambda student: student['score'], reverse=True) # key 매개변수를 람다함수로 설정한다. 학생 딕셔너리 가져와서 score 키값 반환. reverse=True는 내림차순 정렬
...
print(result(20))  # 출력결과: 30 -- 다시 이해할 것. 처음에 outer_func(10)호출해서 x에 10 저장하고 inner_func리턴(10+y가 들어있음) . 그려면 result에 inner_func들어있음. 따라서 print(result(20)하면 inner_func에 20 전달하는 거임  .
"""
...
add, subtract, multiply, divide = calculator() # 튜플을 언패킹. 함수를 사용하려면 튜플을 언패킹해야한다. 튜플의 압축을 푸는 것은 개별 값을 별도의 변수에 할당하는 것을 의미
                                                # 여기서 add, subtract, multiply, divide는 변수임... 함수로 착각했다. 
